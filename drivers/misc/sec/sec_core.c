...
int sec_init(void)
{
    ...
    sec_buffer = (u8*) kmalloc(SIZE_OF_SWRV + 1, GFP_KERNEL);
    memset( (void*) sec_buffer, 0, SIZE_OF_SWRV + 1);
    if (sec_buffer != NULL) {
        SecGetSWRV(sec_buffer);
        printk(KERN_INFO "SecGetSWRV    = ");
        for (i=0; i<SIZE_OF_SWRV; i++) {
            printk("%02x ", sec_buffer[i]);
        }
        printk("\n");
        memset( (void*) sec_buffer, 0, SIZE_OF_SWRV + 1);

        SecGetModelId(sec_buffer);
                printk(KERN_INFO "SecGetModelId = ");
                for (i=0; i<SIZE_OF_MODEL_ID; i++) {
                        printk("%02x ", sec_buffer[i]);
                }
                printk("\n");
        memset( (void*) sec_buffer, 0, SIZE_OF_SWRV + 1);
        ...
        printk(KERN_INFO "SecGetProcID  = ");
        for (i=0; i<SIZE_OF_PROC_ID; i++) {
            printk("%02x ", sec_buffer[i]);
        }
        printk("\n");
        memset( (void*) sec_buffer, 0, SIZE_OF_SWRV + 1);
