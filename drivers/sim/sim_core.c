...
static void sim_module_set_clock_rate (UINT8 reader_id, SIM_MODULE_CLOCK_RATE rate);
...
static volatile SIM_MODULE_REGISTER_BANK *sim_phys_registers[NUM_SIM_MODULES] =
{
    ...
    */
    ...
    switch(cmd)
    {
        case SIM_IOCTL_CONFIGURE_GPIO:
            ...
            write_reg_bits((volatile UINT32 *)CM_AUTOIDLE_WK, SIM_ICLK_AUTOIDLE, SIM_ICLK_AUTOIDLE);
            ...
            write_reg_bits(&(sim_registers[(UINT8)arg]->sysconfig), SOFTRESET_MASK, SOFTRESET_MASK);

            tracemsg ("SIM Module: soft reset SIM module\n\n");
    
            /* block while the module is in reset ... */
            while ((read_reg(&(sim_registers[(UINT8)arg]->sysstatus)) & RESETDONE_MASK) == 0)
            {
                /* yield the processor one time per iteration */
                msleep(1);
                tracemsg ("SIM Module: soft reset not complete yet\n\n");
            }

            /* set the internal SIM clock divider to divide by 2 to obtain
               a SIM clock of 3.75 MHz */
            write_reg_bits(&(sim_registers[(UINT8)arg]->usimconf2), CONFSCLKDIV_MASK, 0);
        ...
        case SIM_IOCTL_READ_DATA_BUFFER:
            status = copy_from_user(args_kernel, (UINT32 *) arg, CMD_PARAMETER_SIZE_4);
        ...
        case SIM_IOCTL_UPDATE_DATA_BUFFER:
            status = copy_from_user(args_kernel, (UINT32 *) arg, CMD_PARAMETER_SIZE_3);

            if (args_kernel[0] < NUM_SIM_MODULES)
            {
                status = copy_from_user(sim_module_card_data[args_kernel[0]].buffer, (UINT8 *)args_kernel[1], args_kernel[2]);
            }
            else
            {
                tracemsg("Warning: Invalid reader ID in SIM driver request.\n");
        ...
        case SIM_IOCTL_INTERRUPT_INIT:
            ...
            if( (UINT8) args_kernel[0] == SIM_MODULE_1)
            {
                ...
                status = request_irq(INT_SIM_GENERAL, sim_module_int_irq_1 , (IRQF_DISABLED | IRQF_NOBALANCING), SIM_DEV_NAME, 0);

                if ( status != 0 )
                {
                    tracemsg("Warning: the SIM driver failed to request an IRQ. IRQ -> %X Error -> %X\n", INT_SIM_GENERAL, status);
                }
            }
            else 
            {
        ...
        case SIM_IOCTL_INTERRUPT_MODE:
            status = copy_from_user(args_kernel, (UINT32 *) arg, CMD_PARAMETER_SIZE_2);
                ...
                switch(args_kernel[1])
                {
                    case SIM_MODULE_RX_MODE:
                        ...
                        error = omap_request_dma(OMAP34XX_DMA_USIM_RX, SIM_DEV_NAME, sim_module_dma_callback,
                                                 (void *)sim_module_card_data[args_kernel[0]].dma_buffer,
                                                 &sim_module_dma_channel);
                        if (error == 0)
                        {
                            /* configure the DMA parameters */      
                            omap_set_dma_transfer_params(sim_module_dma_channel, OMAP_DMA_DATA_TYPE_S8,
                                                         1, SIM_MODULE_MAX_DATA, OMAP_DMA_SYNC_ELEMENT,
                                                         OMAP34XX_DMA_USIM_RX, OMAP_DMA_SRC_SYNC);
                            omap_set_dma_src_params(sim_module_dma_channel, 0, OMAP_DMA_AMODE_CONSTANT,
                                                    (unsigned long)(&(sim_phys_registers[args_kernel[0]]->usim_drx)), 0, 0);
                            omap_set_dma_dest_params(sim_module_dma_channel, 0, OMAP_DMA_AMODE_POST_INC,
                                                     (unsigned long)sim_module_card_data[args_kernel[0]].dma_buffer, 0, 0);
                        ...
                        if (error != 0)
                        {
                            tracemsg ("SIM MODULE DMA CONFIGURATION ERROR -> %X \n", error);
                        }
                    ...
                    case SIM_MODULE_RESET_DETECT_MODE:
                        ...
                        sim_module_card_data[args_kernel[0]].buffer_index = 0;
                        ...
                        sim_module_interrupt_mode = SIM_MODULE_RESET_DETECT_MODE;
                        break;
                    default:
                        tracemsg("Warning: Invalid SIM interrupt mode. \n");
                        break;
                }
            }
            else
        ...
        case SIM_IOCTL_WRITE_SIM_REG_DATA:
            ...
            tracemsg("Write SIM module register data -> reader ID: %X register: %X data: %X\n", args_kernel[0], args_kernel[1], args_kernel[2]);

            if (args_kernel[1] <= sizeof(SIM_MODULE_REGISTER_BANK)) 
            {
                if (args_kernel[0] < NUM_SIM_MODULES)
                {
                    sim_module_register_ptr = (UINT32 *) ((UINT32)sim_registers[args_kernel[0]] + (UINT32)args_kernel[1]);
                }
                else
                {
                    tracemsg("Warning: Invalid reader ID in SIM driver request.\n");
                ...
                {
                    write_reg (sim_module_register_ptr, args_kernel[2]);
        ...
        case SIM_IOCTL_READ_SIM_REG_DATA:
            ...
            if (args_kernel[1] <= sizeof(SIM_MODULE_REGISTER_BANK)) 
            {
                ...
                if(sim_module_register_ptr != NULL)
                {
                    args_kernel[2] = read_reg(sim_module_register_ptr);
        ...
        case SIM_IOCTL_WRITE_SIM_REG_BIT:
            ...
            if (args_kernel[2] <= sizeof(SIM_MODULE_REGISTER_BANK)) 
            {
                if (args_kernel[0] < NUM_SIM_MODULES)
                {
                    sim_module_register_ptr = (UINT32 *) ((UINT32)sim_registers[args_kernel[0]] + (UINT32)args_kernel[2]);
                }
                else
                {
                    tracemsg("Warning: Invalid reader ID in SIM driver request.\n");
                ...
                if (sim_module_register_ptr != NULL)
                {
                    write_reg_bits(sim_module_register_ptr, args_kernel[1], args_kernel[3]);
        ...
        case SIM_IOCTL_SET_SIM_CLOCK_RATE:
            ...
            tracemsg("Set SIM clock rate, reader id -> %X, rate -> %X\n", args_kernel[0], args_kernel[1]);
        ...
        case SIM_IOCTL_READ_RX_EVENT:
            ...
            if (args_kernel[0] < NUM_SIM_MODULES)
            {
                ...
                if(copy_to_user((UINT32 *)arg, args_kernel, CMD_PARAMETER_SIZE_2))
                {
        ...
        case SIM_IOCTL_READ_BUFFER_INDEX:
            ...
            if (args_kernel[0] < NUM_SIM_MODULES)
            {
                ...
                if (sim_module_dma_channel != 0)
                {
                    ...
                    if (omap_get_dma_dst_pos (sim_module_dma_channel) != 0)
                    {
                        /* set the number of characters received via DMA */
                        args_kernel[1] = (UINT32) (omap_get_dma_dst_pos (sim_module_dma_channel) - sim_module_card_data[args_kernel[0]].dma_buffer);
                    }
                ...
                if (copy_to_user((UINT32 *)arg, args_kernel, CMD_PARAMETER_SIZE_2))
                {
                    tracemsg("Warning: failed to copy data to user-space during buffer index retrieval\n");
                    status = -EFAULT;
                }
            }
            else
        ...
        case SIM_IOCTL_RESET_CARD_READER_DATA:
            status = copy_from_user(args_kernel, (UINT32 *)arg, CMD_PARAMETER_SIZE_5);
        ...
        case SIM_IOCTL_DATA_TX:
            ...
            if (args_kernel[0] < NUM_SIM_MODULES)
            {
                ...
                /* Enable DMA mode*/
                write_reg_bits (&(sim_registers[args_kernel[0]]->usim_fifos), SIM_DMA_MODE_MASK, SIM_DMA_MODE_MASK);
                ...
                for (tx_index=0; tx_index < length; tx_index++)
                {
                    ...
                    write_reg (&(sim_registers[args_kernel[0]]->usim_dtx), 
                               sim_module_card_data[args_kernel[0]].buffer[sim_module_card_data[args_kernel[0]].buffer_index++]);
                }
    
                /* setup the TX threshold value to write more data into the FIFO */
                write_reg_bits (&(sim_registers[args_kernel[0]]->usim_fifos), FIFO_TX_TRIGGER_MASK, (SIM_MODULE_TX_FIFO_SIZE - 1) << 2);
                            
                /* clear the transmit complete interrupt */
                write_reg_bits (&(sim_registers[args_kernel[0]]->irqstatus), USIM_TX_MASK, USIM_TX_MASK);

                /* enable the transmit complete interrupt */
                write_reg_bits (&(sim_registers[args_kernel[0]]->irqenable), USIM_TX_EN_MASK, USIM_TX_EN_MASK);
              
                /* enable the transmitter */
                write_reg_bits (&(sim_registers[args_kernel[0]]->usimconf2), TXNRX_MASK, TXNRX_MASK);
            
                /* if there is just one block of data to send ... */
                if (sim_module_card_data[args_kernel[0]].tx_length <= SIM_MODULE_TX_FIFO_SIZE)
                {
                    sim_module_card_data[args_kernel[0]].buffer_index = 0;

                    /* enable the receiver right away */
                    write_reg_bits (&(sim_registers[args_kernel[0]]->usimconf2), TXNRX_MASK, 0);
        ...
        case SIM_IOCTL_GET_SIM_CLOCK_FREQ:
            ...
            if (args_kernel[0] < NUM_SIM_MODULES)
            {
                ...
                if (copy_to_user((UINT32 *)arg, args_kernel, CMD_PARAMETER_SIZE_2))
                {
                    tracemsg("Warning: failed to copy data to user-space for get frequency\n");
                    status = -EFAULT;
                }
            }
        ...
        case SIM_IOCTL_LOW_POWER_STATE:
            status = copy_from_user(args_kernel, (UINT32 *)arg, (sizeof(UINT32) * 2));
            ...
            if (args_kernel[0] < NUM_SIM_MODULES)
            {
                ...
                { 
                    ...
                    write_reg_bits (&(sim_registers[args_kernel[0]]->usim_fifos), SIM_DMA_MODE_MASK, 0);
...
static int sim_open(struct inode *inode, struct file *file) 
{
    ...
    if (status == 0)
    {
        ...
        sim_module_card_data[SIM_MODULE_1].buffer = (UINT8 *)dma_alloc_coherent(NULL, SIM_MODULE_MAX_DATA,
        ...
        if ((sim_module_card_data[SIM_MODULE_1].buffer == NULL) ||
            (sim_module_card_data[SIM_MODULE_1].dma_buffer == 0))
        {
            tracemsg ("The data buffer was not allocated buffer -> %X, dma_buffer -> %X\n",
                    (unsigned int)sim_module_card_data[SIM_MODULE_1].buffer, (unsigned int)sim_module_card_data[SIM_MODULE_1].dma_buffer);
            status = -EFAULT;
        }
    }
...
static void sim_module_int_tx (UINT8 reader_id)
{
    ...
    for (tx_index = 0; tx_index < bytes_remaining; tx_index++)
    {
        ...
        write_reg (&(sim_registers[reader_id]->usim_dtx), sim_module_card_data[reader_id].
                   buffer[sim_module_card_data[reader_id].buffer_index++]);

    }

    /* setup the TX threshold value to write more data into the FIFO */
    write_reg_bits (&(sim_registers[reader_id]->usim_fifos), FIFO_TX_TRIGGER_MASK, (SIM_MODULE_TX_FIFO_SIZE - 1) << 2);
            
    /* clear the transmit complete interrupt */
    write_reg_bits (&(sim_registers[reader_id]->irqstatus), USIM_TX_MASK, USIM_TX_MASK);
        
    /* enable the transmit complete interrupt */
    write_reg_bits (&(sim_registers[reader_id]->irqenable), USIM_TX_EN_MASK, USIM_TX_EN_MASK);

    /* enable the transmitter */
    write_reg_bits (&(sim_registers[reader_id]->usimconf2), TXNRX_MASK, TXNRX_MASK);
...
static void sim_module_int_reset_detect (UINT8 reader_id)
{
    ...
    write_reg_bits (&(sim_registers[reader_id]->irqenable), USIM_RX_EN_MASK, 0);

    /* while there is data in the FIFO ... */
    while (((sim_registers[reader_id]->usim_fifos) & FIFORX_EMPTY_MASK) == 0) 
    {
        /* copy the character into the data buffer */
        sim_module_card_data[reader_id].buffer[sim_module_card_data[reader_id].buffer_index++]
            = (read_reg(&(sim_registers[reader_id]->usim_drx)) & USIMDRX_MASK);
    }
...
static irqreturn_t sim_module_int_irq_1 (int irq, void *dev_id)
{
    ...
    if (((read_reg (&(sim_registers[SIM_MODULE_1]->irqstatus)) & USIM_RESENT_MASK) == USIM_RESENT_MASK) &&
        ((read_reg (&(sim_registers[SIM_MODULE_1]->irqenable)) & USIM_RESENT_EN_MASK) == USIM_RESENT_EN_MASK))
    {
        ...
        write_reg (&(sim_registers[SIM_MODULE_1]->irqstatus), USIM_RESENT_MASK);
    }

    /* if the position detect interrupt fired ... */
    if (((read_reg (&(sim_registers[SIM_MODULE_1]->irqstatus)) & USIM_CD_MASK) == USIM_CD_MASK) &&
        ((read_reg (&(sim_registers[SIM_MODULE_1]->irqenable)) & USIM_CD_EN_MASK) == USIM_CD_EN_MASK))
    {
        /* if card status is changing from removed -> present ... */
        if (((read_reg (&(sim_registers[SIM_MODULE_1]->usimstat)) & STATNOCARD_MASK) == STATNOCARD_MASK) &&
            (sim_module_current_pd_state_sim1 == SIM_REMOVED))
        {
            sim_module_rx_event |= SIM_MODULE_EVENT_SIMPD_INSERTION;
            sim_module_current_pd_state_sim1 = SIM_PRESENT;
        }

        /* else if card status is changing from present -> removed ... */

        else if (((read_reg (&(sim_registers[SIM_MODULE_1]->usimstat)) & STATNOCARD_MASK) == 0) &&
            (sim_module_current_pd_state_sim1 == SIM_PRESENT))
        {
            sim_module_rx_event |= SIM_MODULE_EVENT_SIMPD_REMOVAL;
            sim_module_current_pd_state_sim1 = SIM_REMOVED;
        }

        /* else the CD interrupt fired, but the status didn't change, so ignore */
         
        write_reg (&(sim_registers[SIM_MODULE_1]->irqstatus), USIM_CD_MASK);
    }

    /* if the character timer interrupt fired ... */
    if (((read_reg (&(sim_registers[SIM_MODULE_1]->irqstatus)) & USIM_WT_MASK) == USIM_WT_MASK) &&
        ((read_reg (&(sim_registers[SIM_MODULE_1]->irqenable)) & USIM_WT_EN_MASK) == USIM_WT_EN_MASK))
    {
        /* notify user space */
        sim_module_rx_event |= SIM_MODULE_EVENT_WWT_VIOLATION;
        write_reg_bits (&(sim_registers[SIM_MODULE_1]->irqenable), USIM_WT_EN_MASK, 0);
    }    

    /* if the no ATR interrupt fired ... */
    if (((read_reg (&(sim_registers[SIM_MODULE_1]->irqstatus)) & USIM_NATR_MASK) == USIM_NATR_MASK) &&
        ((read_reg (&(sim_registers[SIM_MODULE_1]->irqenable)) & USIM_NATR_EN_MASK) == USIM_NATR_EN_MASK))
    {
        write_reg_bits (&(sim_registers[SIM_MODULE_1]->irqenable), USIM_NATR_EN_MASK, 0);
       /* notify user space */
        sim_module_rx_event |= SIM_MODULE_EVENT_NO_ATR_FLAG;
    }

    /* if one of the data interrupts fired ... */
    if ((((sim_module_interrupt_mode == SIM_MODULE_RX_MODE) || (sim_module_interrupt_mode == SIM_MODULE_RESET_DETECT_MODE)) &&
         ((read_reg (&(sim_registers[SIM_MODULE_1]->irqstatus)) & USIM_RX_MASK) == USIM_RX_MASK) &&
         ((read_reg (&(sim_registers[SIM_MODULE_1]->irqenable)) & USIM_RX_EN_MASK) == USIM_RX_EN_MASK)) ||
          ((sim_module_interrupt_mode == SIM_MODULE_TX_MODE) &&
           ((read_reg (&(sim_registers[SIM_MODULE_1]->irqstatus)) & USIM_TX_MASK) == USIM_TX_MASK) &&
           ((read_reg (&(sim_registers[SIM_MODULE_1]->irqenable)) & USIM_TX_EN_MASK) == USIM_TX_EN_MASK)))
    {
        ...
        else if (sim_module_interrupt_mode == SIM_MODULE_RESET_DETECT_MODE)
    ...
    if (((read_reg (&(sim_registers[SIM_MODULE_1]->irqstatus)) & USIM_RXFULL_MASK) == USIM_RXFULL_MASK) &&
        ((read_reg (&(sim_registers[SIM_MODULE_1]->irqenable)) & USIM_RXFULL_EN_MASK) == USIM_RXFULL_EN_MASK))
    {
        ...
        write_reg (&(sim_registers[SIM_MODULE_1]->irqstatus), USIM_RXFULL_MASK);
        write_reg_bits (&(sim_registers[SIM_MODULE_1]->irqenable), USIM_RXFULL_EN_MASK, 0);
...
static void sim_module_init_rx_mode (UINT8 reader_id)
{
    ...
    if (reader_id < NUM_SIM_MODULES)
    {
        ...
        write_reg_bits (&(sim_registers[reader_id]->usim_fifos), FIFO_RX_TRIGGER_MASK, 0);

        /* enable DMA mode */
        write_reg_bits(&(sim_registers[reader_id]->usim_fifos), SIM_DMA_MODE_MASK, SIM_DMA_MODE_MASK);

        /* indicate the card is in the non-ATR receive mode */
        sim_module_interrupt_mode = SIM_MODULE_RX_MODE;

        /* if there is no data in the FIFO ... */
        if ((read_reg (&(sim_registers[reader_id]->usim_fifos)) & FIFORX_EMPTY_MASK) == FIFORX_EMPTY_MASK)
        {
            /* clear the RX fifo interrupt */
            write_reg (&(sim_registers[reader_id]->irqstatus), (USIM_RX_MASK | USIM_RXFULL_MASK));
        }

        /* enable the RX full interrupt */
        write_reg_bits (&(sim_registers[reader_id]->irqenable), (USIM_RX_MASK | USIM_RXFULL_EN_MASK), (USIM_RX_MASK | USIM_RXFULL_EN_MASK));
...
static void sim_module_set_voltage_level (SIM_MODULE_VOLTAGE_LEVEL level)
{
    ...
    /* power down the pads */
    write_reg_bits((volatile UINT32 *)PBIAS_CONTROL_LITE, PBIASPWRDNZ1, 0);

    /* enable fast I/O */
    write_reg_bits((volatile UINT32 *)PBIAS_CONTROL_LITE, PBIASSPEEDCNTL1, PBIASSPEEDCNTL1);
    ...
    if ((level == SIM_MODULE_VOLTAGE_HIGH) || (level == SIM_MODULE_VOLTAGE_LOW))
    {
        /* the pad bias level doesn't equal the VSIM level ... */
        while ((read_reg((volatile UINT32 *)PBIAS_CONTROL_LITE) & PBIASVMODEERROR1) != 0)
        {
        ...
        /* enable the pad */
        write_reg_bits((volatile UINT32 *)PBIAS_CONTROL_LITE, PBIASPWRDNZ1, PBIASPWRDNZ1);
    ...
    if (reader_id < NUM_SIM_MODULES)
    {
        switch (rate)
        {
            ...
            case SIM_MODULE_CLOCK_4:
                ...
                write_reg_bits(&(sim_registers[reader_id]->usimconf2), CONFSCLKDIV_MASK, 0);
                /* disable the SIM FCLK */
                clk_disable (usim_fck);
                ...
                /* enable the SIM FCLK */
                clk_enable (usim_fck);
                sim_module_clock_frequency = SIM_MODULE_FREQUENCY_4;
                break;
            case SIM_MODULE_CLOCK_5:
                ...
                /* enable the SIM FCLK */
                clk_enable (usim_fck);
                sim_module_clock_frequency = SIM_MODULE_FREQUENCY_5;
                break;
            default:
                tracemsg("Warning: Invalid SIM clock selection.\n");
                break;
        }
    }
...
void sim_module_dma_callback(INT32 lch, UINT16 ch_status, void *data)
{
    ...
    if (ch_status != 0)
    {
        tracemsg("SIM MODULE DMA CALLBACK ERROR lch -> %X, ch_status -> %X\nregister contents: ", lch, ch_status);
...
static int sim_probe(struct platform_device *pdev)
{
    ...
    {
        tracemsg("sim_probe: Unable to get a major for SIM driver\n");
    ...
    if (IS_ERR(sim_class)) 
    {
     ...
     temp_class = device_create(sim_class, NULL, MKDEV(sim_module_major, 0), NULL, SIM_DEV_NAME);
...
int __init sim_init(void)
{
    ...
    else
    {
        ...
        if (ret != 0)
        {
            ...
            tracemsg("sim_init: Device registration failed.\n");
        }
        else
        {
            tracemsg("sim_init: Device registration passed.\n");
        }
    }
