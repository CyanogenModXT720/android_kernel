 ...
 * @(#) sl@belcarra.com/whiskey.enposte.net|otg/platform/otglib/pcd.c|20071024210348|31772
 *
 ...
 * Date               Author           Comment
 * -----------------------------------------------------------------------------
 * 01/14/2009         Motorola         Fix initialization panic when preempt kernel boot up 
 */
...
/* mutex to prevent pcd_register_bh and pcd_deregister_bh race condition */
DECLARE_MUTEX(pcd_mutex);
...
void pcd_urb_finished_irq(struct usbd_urb *urb, int rc)
{
        ...
        if (urb == bus->ep0_urb) {
                ...
                otg_pthread_mutex_lock(&urb->endpoint->mutex);    /*lock mutex*/
                usbd_unlink_urb(urb);
                otg_pthread_mutex_unlock(&urb->endpoint->mutex);  /*lock mutex*/
                return;
        }
                ...
                if ( (endpoint->tx_urb = usbd_first_ready_urb(endpoint, &endpoint->rdy))) {
                        ...
                        if (TRACE_VERBOSE)
                                TRACE_MSG3(pcd->TAG, "[%2x] NEXT TX: length: %d flags: %x", 
                                                endpoint->bEndpointAddress[bus->high_speed],
...
int bus_halt_endpoint (struct usbd_bus_instance *bus, int bEndpointAddress,  int flag)
{
        ...
        TRACE_MSG4(pcd->TAG, "BUS_ENDPOINT HALT:  endpoint: %p bEndpointAddress: %02x flag: %d feature: %04x", 
                        endpoint, bEndpointAddress, flag, endpoint->feature_setting);

        RETURN_EINVAL_UNLESS(endpoint);

        if (flag) {
                UNLESS (endpoint->feature_setting & FEATURE(USB_ENDPOINT_HALT)) {
                        TRACE_MSG1(pcd->TAG, "BUS_ENDPOINT HALTING:  endpoint: %p", (int) endpoint);
                        endpoint->feature_setting = (endpoint->feature_setting | FEATURE(USB_ENDPOINT_HALT));
                        usbd_flush_endpoint(endpoint);
                        return usbd_pcd_ops.halt_endpoint ? usbd_pcd_ops.halt_endpoint(pcd, endpoint, TRUE) : 0;
                }
        }
        else {
                if (endpoint->feature_setting & FEATURE(USB_ENDPOINT_HALT)) {
                        TRACE_MSG1(pcd->TAG, "BUS_ENDPOINT CLEARING:  endpoint: %p", (int) endpoint);
                        endpoint->feature_setting = (endpoint->feature_setting & ~FEATURE(USB_ENDPOINT_HALT));
                        return usbd_pcd_ops.halt_endpoint ? usbd_pcd_ops.halt_endpoint(pcd, endpoint, FALSE) : 0;
                }
        }
        return 0;
        ...
        if (otg_likely ( (int) (rcv_urb = endpoint->rcv_urb))) {
                ...
                if (TRACE_VERBOSE)
                        TRACE_MSG7(pcd->TAG, "[%2x] alloc: %d buffer: %d actual: %d packet: %d transfer: %d len: %d",
                                        endpoint->bEndpointAddress[rcv_urb->bus->high_speed],
                                ...
                                /*&& (len <= endpoint->wMaxPacketSize)*/)
                {
                        ...
                        if (
                                        (
                                         ...
                                         (rcv_urb->actual_length + endpoint->wMaxPacketSize[bus->high_speed] > rcv_urb->alloc_length) ||
										  (len < MAX_COMBINE_TRANSFERSIZE && omaphs_epn(endpoint) != 0)))
                        {
        ...
        if ( (tx_urb = endpoint->tx_urb)) {
                ...
                if (TRACE_VERBOSE)
                        TRACE_MSG6(pcd->TAG, "[%2x] BUS_TX CURRENT TX_URB: %p actual: %d sent: %d last: %d: flags: %x",
                                        endpoint->bEndpointAddress[tx_urb->bus->high_speed],
                ...
                if (otg_likely (!restart)) {
                        ...
                        TRACE_MSG1(pcd->TAG, "[%2x] RESTARTING", endpoint->bEndpointAddress[tx_urb->bus->high_speed]);
                }
                endpoint->last = 0;
...
void pcd_bus_event_handler_irq (struct usbd_bus_instance *bus, usbd_device_event_t event, int data)
{
        ...
        case DEVICE_BUS_ACTIVITY:
                ...
                if((bus->device_state == STATE_CONFIGURED) && (bus->status == USBD_OK)){
...
void pcd_dp_pullup_func(struct otg_instance *otg, u8 flag)
{
        ...
        switch (flag) {
        case SET:
                TRACE_MSG1(pcd->TAG, "PCD PULLUP flag: %d SET", flag);
                usbd_pcd_ops.softcon(pcd, TRUE);
                break;
        case RESET:
                TRACE_MSG1(pcd->TAG, "PCD PULLUP flag: %d RESET", flag);
                usbd_pcd_ops.softcon(pcd, FALSE);
                break;
        }
}
