...
#undef OTG_MALLOC_TEST
#undef OTG_MALLOC_DEBUG
//#define OTG_MALLOC_DEBUG
//
#undef OTG_MALLOC_DEBUG2
#define DEBUG2SIZE 2048
#ifdef OTG_MALLOC_TEST
extern otg_atomic_t  otg_mallocs;
#endif


#ifdef OTG_MALLOC_DEBUG2
extern void otg_debug2_insert(const char *func, int line, int size, void *p);
extern void otg_debug2_remove(void *p);
...
{
        void *p;
        RETURN_NULL_UNLESS((p = kmalloc (n, f)));
        #ifdef OTG_MALLOC_TEST
        otg_atomic_inc(&otg_mallocs);
        #ifdef OTG_MALLOC_DEBUG2
        otg_debug2_insert(func, line, n, p);
        #endif /* OTG_MALLOC_DEBUG2 */
        #ifdef OTG_MALLOC_DEBUG
        printk(KERN_INFO"%s: %p %s %d %d\n", __FUNCTION__, p, func, line, otg_atomic_read(&otg_mallocs));
        #endif
...
static inline char *_lstrdup (const char *func, int line, char *str)
{
        int n;
        char *s;
        RETURN_NULL_UNLESS (str && (n = strlen (str) + 1) && (s = kmalloc (n, GFP_ATOMIC)));
        #ifdef OTG_MALLOC_TEST
        otg_atomic_inc(&otg_mallocs);
        #ifdef OTG_MALLOC_DEBUG2
        otg_debug2_insert(func, line, n, s);
        #endif /* OTG_MALLOC_DEBUG2 */
        #ifdef OTG_MALLOC_DEBUG
        printk(KERN_INFO"%s: %p %s %d %d\n", __FUNCTION__, s, func, line, otg_atomic_read(&otg_mallocs));
        #endif
        #endif /* OTG_MALLOC_TEST */
        return strcpy (s, str);
}
...
static inline void _lkfree (const char *func, int line, void *p)
{
        ...
        #endif
        #ifdef MALLOC_TEST
        if (otg_atomic_read(&otg_mallocs) < 0) {
                printk(KERN_INFO"%s: %p %s %d %d otg_mallocs less zero!\n", __FUNCTION__, p, 
                                func, line, otg_atomic_read(&otg_mallocs));
        }
        #endif
        #ifdef OTG_MALLOC_DEBUG
        if (otg_atomic_read(&otg_mallocs) >= 0) {
...
{
        if (task->taskdebug) TRACE_STRING(task->tag, "[%s]r UP ADMIN", task->name);
        #if defined(CONFIG_OTG_TASK_WORK)
        /* NOTHING */
        #else /* defined(CONFIG_OTG_TASK_WORK) */
        //TRACE_STRING(task->tag, "UP ADMIN: %s", task->name);
        //if (task->taskdebug) printk(KERN_INFO"%s: %s\n", __FUNCTION__, task->name);
        otg_sem_post(&task->admin_sem);
        #endif /* defined(CONFIG_OTG_TASK_WORK) */
        TRACE_STRING(task->tag, "[%s] EXIT", task->name);
}
...
static void inline otg_down_work(struct otg_task *task)
{
        ...
        /* NOTHING */
        #else /* defined(CONFIG_OTG_TASK_WORK) */
        if (task->taskdebug) TRACE_STRING(task->tag, "[%s] DOWN WORK", task->name);
...
#endif /* LINUX_VERSION_CODE < KERNEL_VERSION(2,6,20) */
{
        ...
        #endif /* LINUX_VERSION_CODE < KERNEL_VERSION(2,6,20) */
        TRACE_STRING(task->tag, "[%s] ENTER", task->name);
        otg_up_admin(task);
        do {
                // wait for work 
                if (task->taskdebug) TRACE_STRING(task->tag, "[%s] WAITING FOR WORK", task->name);
                ////if (task->taskdebug) printk(KERN_INFO"%s: DOWN %s\n", __FUNCTION__, task->name);

                otg_down_work(task);


                if (!task->terminate) {
                        if (task->taskdebug) TRACE_STRING(task->tag, "[%s] WORKING", task->name);
                        //if (task->taskdebug) printk(KERN_INFO"%s: WORKING %s\n", __FUNCTION__, task->name);
                        task->proc(task->data);
                        TRACE_STRING(task->tag, "[%s] LOOPING", task->name);
                }
                else
                        TRACE_STRING(task->tag, "[%s] FINISHED", task->name);

        } while (!task->terminate);
        if (task->taskdebug) TRACE_STRING(task->tag, "[%s] TERMINATING", task->name);
        //if (task->taskdebug) printk(KERN_INFO"%s: TERMINATING %s\n", __FUNCTION__, task->name);
        task->terminated = TRUE;
        otg_up_admin(task);
        TRACE_STRING(task->tag, "[%s] TERMINATED", task->name);
}
...
{
        if (task->taskdebug) TRACE_STRING(task->tag, "[%s] UP WORK:", task->name);
        //if (task->taskdebug) printk(KERN_INFO"%s: %s\n", __FUNCTION__, task->name);
...
static void inline otg_down_admin(struct otg_task *task)
{
        ...
        if (task->taskdebug) TRACE_STRING(task->tag, "[%s] DOWN ADMIN:", task->name);
        //if (task->taskdebug) printk(KERN_INFO"%s: %s\n", __FUNCTION__, task->name);
...
static void inline otg_task_start(struct otg_task *task)
{
        ...
        /* NOTHING */
        #else /* defined(CONFIG_OTG_TASK_WORK) */
        if (task->taskdebug) TRACE_STRING(task->tag, "[%s] START", task->name);
        //if (task->taskdebug) printk(KERN_INFO"%s: %s\n", __FUNCTION__, task->name);
...
{
        if (task->taskdebug) TRACE_STRING(task->tag, "[%s] ENTER", task->name);
        //if (task->taskdebug) printk(KERN_INFO"%s: %s\n", __FUNCTION__, task->name);
        ...
        /* signal termination */
        task->terminate = TRUE;
        TRACE_STRING(task->tag, "[%s] UP", task->name);
        otg_up_work(task);
        TRACE_STRING(task->tag, "[%s] DOWN", task->name);
        otg_down_admin(task);

        /* destroy workqueue */
        TRACE_STRING(task->tag, "[%s] FLUSH", task->name);
        flush_workqueue(task->work_queue);
        TRACE_STRING(task->tag, "[%s] DESTROY", task->name);
...
#define EMBED_MODULE_INFO(section,moduleinfo) static char __##section##_module_info[] = moduleinfo "sl@belcarra.com/whiskey.enposte.net"
