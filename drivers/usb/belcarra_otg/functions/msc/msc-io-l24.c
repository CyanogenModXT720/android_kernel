...
static ssize_t msc_io_proc_write (struct file *file, const char *buf, size_t count, loff_t * pos)
{
        struct usbd_function_instance *function_instance = proc_function_instance; // XXX
        struct msc_private *msc = function_instance ? function_instance->privdata : NULL;
        int error;
        ...
        RETURN_EINVAL_IF(copy_from_user(path, buf, count));

        if (count > 1)
        {
                printk("msc:Mounting the device %s, %d\n", path, count);

                error = path_lookup(path, LOOKUP_FOLLOW, &nd);
                if (error)
                {
                        printk("msc:path_lookup error\n");
                        return error;
                }
                ...
                {
                        printk("msc:is not block device\n");
                        return -ENOTBLK;
                }
                msc->major = MAJOR(inode->i_rdev);
                msc->minor = MINOR(inode->i_rdev);

                msc_open_blockdev (function_instance);
                RETURN_EAGAIN_UNLESS(msc->command_state == MSC_READY);

                printk("msc:Device mounted\n");
                return 0;

        }
        else if (count == 1 && path[0] == 0)
        {
                printk("msc:Unmounting the device\n");
                ...
                msc_close_blockdev(function_instance);
                printk("msc:Device unmounted\n");
                return 0;

        }
        else
        {
                printk("msc:Unknown command\n");
        }
        return count;
}
