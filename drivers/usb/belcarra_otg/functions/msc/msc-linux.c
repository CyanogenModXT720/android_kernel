 ...
 * Date               Author           Comment
 * -----------------------------------------------------------------------------
 * 10/15/2008         Motorola         Changes for kernel API changes
 * 12/16/2008         Motorola         Change the order of submit_bio and recv urb to avoid mmc panic
 *
 */
...
#define MAX_DATA_IN_URB         (PAGE_SIZE*8)
...
/*! submit_write_ready_bio - submit current ready bio, which is already contain data copied from urb buffer
 *                           also add this bio instance to ongoing bio list.
 * @param msc - pointer to msc structure
 * @return none
 */

void submit_write_ready_bio(struct msc_private *msc)
{
  struct wr_bio* p_wr_ready_bio = msc->curr_wr_bio;

  spin_lock(&(msc->wrbio_ongoing_lock));
  list_add_tail(&(p_wr_ready_bio->list_item), &(msc->wrbio_ongoing_list));
  spin_unlock(&(msc->wrbio_ongoing_lock));
  submit_bio(WRITE, &(p_wr_ready_bio->msc_bio));

}

/*! get_free_write_bio - get a free bio instance from free bio list
 * @param msc - pointer to msc structure 
 * @return wr_bio - a pointer to bio instance
 */

struct wr_bio* get_free_write_bio(struct msc_private* msc)
{
  struct wr_bio* free_wr_bio = NULL;

  spin_lock(&(msc->wrbio_free_lock));
  if(!list_empty(&(msc->wrbio_free_list))){
    free_wr_bio = list_entry(msc->wrbio_free_list.next, struct wr_bio, list_item);
    list_del(msc->wrbio_free_list.next);
  }
  spin_unlock(&(msc->wrbio_free_lock));

  return free_wr_bio;
}

/*! put_free_write_bio - remove bio instance from ongoing bio list and add it to free bio list
 * @param msc - pointer to msc structure
 *        p_finish_bio - a pointer to bio structure
 * @return none
 */

void put_free_write_bio(struct msc_private* msc, struct bio* p_finish_bio)
{
  struct wr_bio* p_wr_bio;
  /* remove this bio from ongoing bio list */
  spin_lock(&(msc->wrbio_ongoing_lock));
  list_for_each_entry(p_wr_bio, &(msc->wrbio_ongoing_list), list_item){
    if(&(p_wr_bio->msc_bio) == p_finish_bio){
      list_del(&(p_wr_bio->list_item));
      break;
    }
  }
  if((p_wr_bio->list_item).next == &(msc->wrbio_ongoing_list)){
    spin_unlock(&(msc->wrbio_ongoing_lock));
    return;
  }
  spin_unlock(&(msc->wrbio_ongoing_lock));

  /* set bio.bi_nex to NULL */
  p_wr_bio->msc_bio.bi_next = NULL;

  /* add this bio into free bio list */
  spin_lock(&(msc->wrbio_free_lock));
  list_add_tail(&(p_wr_bio->list_item), &(msc->wrbio_free_list));
  spin_unlock(&(msc->wrbio_free_lock));
}
...
void msc_open_blockdev(struct usbd_function_instance *function_instance)
{
	...
	msc->max_blocks = MAX_DATA_IN_URB / msc->block_size;
	...
	msc->read_bio.bi_private = function_instance;

        msc->read_bio.bi_io_vec=&msc->rbio_vec;
        msc->rbio_vec.bv_page = alloc_pages(GFP_NOIO, 3);     // XXX ensure that this gets de-allocated
	...
	/* init 2 write bio lists head */
	INIT_LIST_HEAD(&(msc->wrbio_free_list));
        INIT_LIST_HEAD(&(msc->wrbio_ongoing_list));
        msc->curr_wr_bio = NULL;
	/* intialize  write bio structure */
        for(i = 0; i < WR_BIO_NUM; i++){
          bio_init(&(msc->write_bio[i].msc_bio));
          msc->write_bio[i].index = i;
          msc->write_bio[i].msc_bio.bi_private = function_instance;
          msc->write_bio[i].msc_bio.bi_io_vec=&msc->wbio_vec[i];
	  msc->wbio_vec[i].bv_page = alloc_pages(GFP_NOIO, 3);     // XXX ensure that this gets de-allocated
          msc->wbio_vec[i].bv_len = 0;
          msc->wbio_vec[i].bv_offset = 0;
          msc->write_bio[i].msc_bio.bi_vcnt = 1;
          msc->write_bio[i].msc_bio.bi_idx = 0;
          msc->write_bio[i].msc_bio.bi_size = 0;
          msc->write_bio[i].msc_bio.bi_bdev = msc->bdev;
          msc->write_bio[i].msc_bio.bi_sector = 0;
          list_add(&(msc->write_bio[i].list_item), &(msc->wrbio_free_list));
          msc->delay_flag = 0;
        }
...
void msc_close_blockdev(struct usbd_function_instance *function_instance)
{
	...
	int i;
...

        {
                ...
                wait_event_interruptible_timeout(msc1_wq, ((msc->read_pending == 0 && msc->write_pending == 0)), 20);
        }
...
#if defined(LINUX26)
        __free_pages(msc->rbio_vec.bv_page, 3);
        msc->rbio_vec.bv_page = NULL;
	for(i = 0; i < WR_BIO_NUM; i++){
	  __free_pages(msc->wbio_vec[i].bv_page, 3);
          msc->wbio_vec[i].bv_page = NULL;
	}
...
static void msc_block_read_finished(struct bio *bio, int err);
...
static void msc_block_read_finished(struct bio *bio, int err)
{
        ...
        int rc;
        int i;
        int bytes_done1, bytes_done=msc->rbio_vec.bv_len;
        char *btemp_data;
...
int msc_in_read_10_urb_sent(struct usbd_urb *tx_urb)
{
        ...
        if (MSC_BLOCKIO_FINISHED & msc->io_state) {
                ...
                // XXX uptodate?
#if defined(LINUX26)
                msc_block_read_finished(&msc->read_bio, msc->err);
#else /* defined(LINUX26) */
...
#if defined(LINUX26)
static void msc_data_written(struct bio *bio, int err);
#else /* defined(LINUX26) */
...
int msc_scsi_write_10(struct usbd_function_instance *function_instance, char *name, int op)
{
	...
	//printk("RECV WRITE lba: %x    ", msc->lba);
        //printk("RECV WRITE blocks: %d\n", msc->TransferLength_in_blocks);
...
void msc_recv_out_blocks(struct usbd_urb *rcv_urb)
{
	...
	if(msc->curr_wr_bio == NULL){
	  /* if the current write bio pointer is NULL, get a free write bio from free bio list.*/
          msc->curr_wr_bio = get_free_write_bio(msc);
          if(msc->curr_wr_bio == NULL){
	    /* if there is no bio available, can't receive. halt end point and return.  */
	    usbd_halt_endpoint(function_instance, BULK_OUT);
	    msc->data_transferred_in_bytes=msc->TransferLength_in_bytes;
	    msc->command_state = MSC_CBW_PASSED;
            return;
	...
	/* get bio buffer virtural memory address, and copy urb buffer data to bio buffer.*/
	btemp_data = page_address(msc->curr_wr_bio->msc_bio.bi_io_vec->bv_page);
	...
	msc->curr_wr_bio->msc_bio.bi_io_vec->bv_offset = 0;
        msc->curr_wr_bio->msc_bio.bi_vcnt = 1;
        msc->curr_wr_bio->msc_bio.bi_idx = 0;
        msc->curr_wr_bio->msc_bio.bi_size = TransferLength_in_bytes;
        msc->curr_wr_bio->msc_bio.bi_io_vec->bv_len =  TransferLength_in_bytes;
        msc->curr_wr_bio->msc_bio.bi_bdev = msc->bdev;
        msc->curr_wr_bio->msc_bio.bi_sector = msc->lba;
        msc->curr_wr_bio->msc_bio.bi_end_io = msc_data_written;
        msc->io_state |= MSC_BLOCKIO_PENDING;
        msc->write_pending=1;
        ...
        /*Case 11 Ho > Do */
		  ...
		  msc->curr_wr_bio->msc_bio.bi_size =  msc->TransferLength_in_bytes;
                        msc->curr_wr_bio->msc_bio.bi_io_vec->bv_len = msc->TransferLength_in_bytes;
                        msc->caseflag=11;
         ...
         */
	if(msc->curr_wr_bio->msc_bio.bi_size){
	        /* submit bio already contain urb data to bio layer */
	        submit_write_ready_bio(msc);
                generic_unplug_device(bdev_get_queue(msc->bdev));
		/* After submit, reset to current write bio pointer to NULL to prepare for receive next urb data.*/
		msc->curr_wr_bio = NULL;
		if(msc->caseflag == 12 && msc->command_state == MSC_DATA_OUT_WRITE_FINISHED){
                  if(!list_empty(&(msc->wrbio_free_list))){
		    /* if all the data has been received, and there is free bio available,
		     send USB_MSC_PASSED immediately. Ready to receive further data from USB bus.*/
		    msc_start_sending_csw(function_instance, USB_MSC_PASSED);
		  }
                  else
		    /* if all the data has been received, but there is no free bio available,
		     set the delay flag to 1, USB_MSC_PASSED message will be sent in bio callback
		    function. Further data will be received from USB bus after bio callback return.
		    At that time, bio callback has added finished bio to free bio list.*/
		    msc->delay_flag = 1;

                }
                else if(msc->command_state != MSC_DATA_OUT_WRITE_FINISHED){
                  if(!list_empty(&(msc->wrbio_free_list))){
		    /* if has not finish received all data, and there is free bio available,
		     start to receive next data block from USB bus.*/
		    msc_start_receiving_data(function_instance);
		  }
                  else
		    /* if has not finish received all data, and there is no free bio available,
		     set delay flag to 1. Next data block will be received after bio callback return.
		    At that time, bio callback has added finished bio to free bio list.*/
                    msc->delay_flag = 1;
...
static void msc_data_written(struct bio *bio, int err)
{
        ...
        TRACE_MSG3(MSC,"DATA WRITTEN err: %x state: %x io: %x", err, msc->command_state, msc->io_state);

        TRACE_MSG1(MSC,"DATA WRITTEN lba: %x", msc->lba);

	/* return this finish bio to free bio list */
	put_free_write_bio(msc, bio);
	...
	local_irq_save(flags);
		...
		*/
        ...
        if (MSC_DATA_OUT_WRITE_FINISHED == msc->command_state) {
                ...
                if (msc->caseflag==12 && msc->delay_flag == 1){
		  /* if all data has been received, and delay flag has been set due to lack of availble bio.
		   send USB_MSC_PASSED to PC now. Because free bio has alreay been available due to previous put_free_write_bio
		   call in this function. Reset delay flag before return.*/
		...
		else if (msc->command_state != MSC_DATA_OUT_WRITE_FINISHED && msc->delay_flag == 1)
		{
		  /* if there is further data block has not yet been received, and delay flag has been set
		   due to lack of available bio. Start to receive next data block now. Because free bio has
		  alreaday been available due ot previous put_free_write_bio call in this function. Reset delay flag before return.*/
			msc_start_receiving_data(function_instance);
			msc->delay_flag = 0;
		}
        return;
