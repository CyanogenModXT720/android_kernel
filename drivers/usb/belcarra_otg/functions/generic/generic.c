...
#define MAX_SERIAL_NUMBER_BYTES 16
static char devSerialNumber[MAX_SERIAL_NUMBER_BYTES+1] = {0};
...
{
	int i;
	/*Read the Serial Number from the Die*/
	...
	sprintf(devSerialNumber, "%08x%08x", val[1], val[0]);
	devSerialNumber[MAX_SERIAL_NUMBER_BYTES] = 0;
	...
	for (i = 0; ; i++) {
		...
		BREAK_UNLESS(config->interface_names);
                generic_register(config, generic_config_name, override, idVendor, idProduct, devSerialNumber, tag);
	}
	return 0;
}
...
void generic_modexit (struct generic_config *generic_configs, otg_tag_t tag)
{
	...
	for (i = 0; ; i++) {
		 ...
		 */
                ...
                for (j=0;j<config->interfaces;j++) {
			np = (char *) config->interface_list[j];
			LKFREE (np);
		}
