 ...
 * @(#) sl@belcarra.com/whiskey.enposte.net|otg/functions/network/net-l24-os.c|20071005061152|47446
 *
...
STATIC struct net_device_stats *network_get_stats (struct net_device *net_device)
{
        ...
        TRACE_MSG2(NTT, "NETWORK npd: %p net_device_stats: %p", npd, npd ? npd->net_device_stats : NULL);
...
///
void init_net_device(struct net_device *dev)
{
        dev->get_stats = network_get_stats;
        dev->tx_timeout = network_tx_timeout;
        dev->do_ioctl = network_do_ioctl;
        dev->set_config = network_set_config;
        dev->set_mac_address = network_set_mac_addr;
        dev->hard_start_xmit = network_hard_start_xmit;
        dev->change_mtu = network_change_mtu;
        dev->init = network_init;
        dev->uninit = network_uninit;
        dev->open = network_open;
        dev->stop = network_stop;
        //dev->priv = NULL;
}
...
{
        int rc = 0;
        struct socket  temp_skt;
        mm_segment_t save_get_fs = get_fs();
        TRACE_MSG1(NTT, "cmd: %x", cmd);
        set_fs(get_ds());

        // In Kernel 2.6.x, inet_ioctl() is exported from Kernel.
        // Based only current Belcarra used cases, it is ok if the first parameter is 0.
        // rc = local_devinet_ioctl(cmd, ifreq);
        rc = inet_ioctl(&temp_skt, (unsigned int)cmd, (unsigned long)((void *)ifreq));
        ...
        while (TRUE) {
...

                {
                        ...
                        if ((config_attached != npd->config_status) && npd->ip_addr) {
                                ...
                                network_attach (npd, npd->ip_addr, npd->network_mask, npd->router_ip, 1);
                                TRACE_MSG0(NTT,"ATTACHED");
                ...
                else
                {
                        if (config_detached != npd->config_status) {
                                ...
                                network_attach (npd, npd->ip_addr, npd->network_mask, npd->router_ip, 0);
                                TRACE_MSG0(NTT,"DETACHED");
                                npd->config_status = config_detached;
                                continue;
                        }

                }
                TRACE_MSG0(NTT,"BREAK");
                break;
        }
        //sleep(1);
        TRACE_MSG0(NTT,"EXIT");
...
STATIC int network_create(void)
{
        ...
        Network_net_device = alloc_netdev ( 0, "usbl0", init_net_device);
...
STATIC void network_destroy(void)
{
        ...
        netif_carrier_off(Network_net_device);
        unregister_netdev(Network_net_device);
        free_netdev(Network_net_device);
        ...
        #if defined(CONFIG_OTG_NETWORK_BLAN_AUTO_CONFIG) || defined(CONFIG_OTG_NETWORK_RARPD_AUTO_CONFIG)
        if (npd->config_otgtask) {
                TRACE_MSG0(NTT, "calling otg_task_exit(config_tgtask)");
                otg_task_exit(npd->config_otgtask);
                TRACE_MSG0(NTT, "otg_task_exit finished");
                npd->config_otgtask = NULL;
        }
        #endif /* defined(CONFIG_OTG_NETWORK_BLAN_AUTO_CONFIG) || defined(CONFIG_OTG_NETWORK_RARPD_AUTO_CONFIG) */
        #ifdef CONFIG_OTG_NETWORK_HOTPLUG
