 ...
 * @(#) balden@belcarra.com/seth2.rillanon.org|otg/functions/network/blan-if.c|20070924203247|39017
 *
...
int net_fd_recv_urb_mdlm(struct usbd_urb *urb, int rc)
{
        ...
        if (1 == (len % urb->wMaxPacketSize)) {
                ...
                if (CRC32_GOOD != crc) {
                        ...
                        //printk(KERN_INFO"%s: B CRC 0x%x 0x%x\n", __FUNCTION__, CRC32_GOOD, crc); 
                        npd->seen_crc_error = 1;
                        THROW_IF(npd->seen_crc, crc_error);
                }
                else
