...
int usb_vendor_request(struct usbd_device_request * request)
{
   ...
   // Verify that this is a vendor request.
...
int usbd_get_descriptor (struct usbd_function_instance *function, u8 *buffer, int wLength, int descriptor_type, int index)
{
        ...
        int actual_length = 0;
        unsigned short temp_vid, temp_pid;
        ...
        case USB_DT_DEVICE: {
                ...
                temp_vid = otg_message_cfg_vid();
                temp_pid = otg_message_cfg_pid();
                if ((temp_vid != 0 && (temp_pid != 0))) {
                        device_descriptor.idVendor = cpu_to_le16(temp_vid);
                        device_descriptor.idProduct = cpu_to_le16(temp_pid);
                }
                else {
                ...
                if (otg_message_cfg_cdrom())
                        device_descriptor.idProduct = cpu_to_le16(0x41DE);
...
static int do_standard_device_request (struct usbd_function_instance *function, struct usbd_device_request *request)
{
		...
		urb->irq_flags = 0;
                ...
                case USB_REQ_GET_DESCRIPTOR:
                        ...
                        otg_require_switch(GET_DESCRIPTOR_SWITCH);
                        break;
