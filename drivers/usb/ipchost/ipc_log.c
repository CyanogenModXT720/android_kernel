...
#define IPC_LOG_URB_START()                                                             \
    ...
    if ((ipc_log_param.write_buf != NULL ) && (ipc_log_param.urb_flag == 0))   {        \
        ...
        if( usb_submit_urb(&ipc_log_param.read_urb, GFP_KERNEL) ) {                     \
...
static int usb_ipc_log_open(struct inode * inode, struct file * file)
{
	...
	printk("%s: USB IPC LOG open error\n", __FUNCTION__);
	return -EBUSY;
    }
 ...
 *****************************************************************************/
static unsigned int usb_ipc_log_poll(struct file * file, struct poll_table_struct *wait)
{
    unsigned int mask = 0;

    DEBUG("Enter %s\n", __FUNCTION__);
    if( ( ipc_log_param.isopen == 0 ) || (ipc_log_param.probe_flag == 0) ) {
        printk("%s: USB IPC LOG calling poll error\n", __FUNCTION__);
        return -EBUSY;
    }
 ...
 *****************************************************************************/
static ssize_t usb_ipc_log_read(struct file * filp, char __user * buf, size_t count, loff_t * l)
{       
    int size;
...
static struct file_operations usb_ipc_log_fops = {
	owner:          THIS_MODULE,
 ...
 */
int usb_ipc_log_probe(struct usb_interface *intf, const struct usb_device_id *id)
{	
	...
	if (!IS_EP_BULK_IN(ipc_endpoint)) 	{			
		printk("%s: Undetected IN endpoint\n", __FUNCTION__);
		return -ENOMEM;	/* Shouldn't ever get here unless we have something weird */
	}
	ipc_log_param.udev         = dev;
	ipc_log_param.read_bufsize = (MAX_LOG_BUF_SIZE > ipc_endpoint->wMaxPacketSize)? MAX_LOG_BUF_SIZE : ipc_endpoint->wMaxPacketSize;
	temp_buf = ipc_log_alloc_read_buffer();
	if(temp_buf == NULL)  {
		return -ENODEV;
	}
	...
	if( retval )   {
		printk("%s: Register USB Log device failed\n", __FUNCTION__);
		return -ENODEV;
	}

	usb_fill_bulk_urb(&ipc_log_param.read_urb, dev, usb_rcvbulkpipe(dev,ipc_endpoint->bEndpointAddress), 0, 0, ipc_log_read_callback, 0);
