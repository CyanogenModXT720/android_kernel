/*
 * drivers/video/omap2/misc/dispsw.h
 *
...
#ifdef DEBUG
#define DBG(format, ...) \
	printk("DISPSW: " format, ## __VA_ARGS__)
...
enum dispsw_leave_image {
	DISPSW_UNLEAVE_IMAGE,
	DISPSW_LEAVE_IMAGE,
};
...
struct dispsw_cmd {
	...
	/* Only used for the DISPSW_CMD_RESOLUTION command
	 */
	char resolution_name[DISPSW_MAX_NAME_SIZE + 1];
	enum dispsw_leave_image leave_image;
};

struct dispsw_position {
    int x;
    int y;
};
...
#define DISPSW_QUERYRES		_IOWR(DISPSW_IOCTL_MAGIC, \
				    DISPSW_IOCTL_BASE+0, struct dispsw_res_info)
#define DISPSW_S_CMD		_IOW(DISPSW_IOCTL_MAGIC, \
				    DISPSW_IOCTL_BASE+1, struct dispsw_cmd)
#define DISPSW_SETGFXPOSITION	_IOW(DISPSW_IOCTL_MAGIC, \
				    DISPSW_IOCTL_BASE+2, struct dispsw_position)
#endif /* __DISPSW_H__ */
