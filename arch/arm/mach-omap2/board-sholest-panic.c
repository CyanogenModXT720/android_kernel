...
static void sholest_panic_erase_callback(struct erase_info *done)
{
...
erase:
	...
	erase.mtd = mtd;
	erase.callback = sholest_panic_erase_callback;
	...
	for (i = 0; i < mtd->size; i += mtd->erasesize) {
		...
		if (rc) {
			printk(KERN_WARNING
			       "sholest_panic: Skipping erase of bad "
			       "block @%llx\n", erase.addr);
		...
		if (rc) {
			...
			remove_wait_queue(&wait_q, &wait);
			printk(KERN_ERR
			       "sholest_panic: Erase of region 0x%llx, 0x%llx failed\n",
	...
	printk(KERN_DEBUG "sholest_panic: Panic partition cleared\n");
out:
	return;
}
